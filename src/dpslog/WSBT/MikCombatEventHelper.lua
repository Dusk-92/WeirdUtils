...
MikCEH.recentlySelectedPlayers = {};
local recentlySelectedPlayers = MikCEH.recentlySelectedPlayers;
local elapsedTime = 0;
...
-- strfind = function () return nil end
...
function MikCEH.RegisterEvents()
  -- Register all events (combat parse + core).
...
-- **********************************************************************************
-- Events that CLEU replaces (combat damage/heal/miss/dot/shield/pet/aura parsing).
-- Notification-only events (honor, XP, rep, skill, item buffs, buff fades) are NOT
-- replaced by CLEU and stay registered always.
-- **********************************************************************************
MikCEH.cleuReplacedEvents = {
...
-- Build lookup for fast profiling checks
MikCEH.cleuReplacedLookup = {}
for _, ev in ipairs(MikCEH.cleuReplacedEvents) do
  MikCEH.cleuReplacedLookup[ev] = true
end

function MikCEH.RegisterCombatParseEvents()
  for _, ev in ipairs(MikCEH.cleuReplacedEvents) do
    MCEHEventFrame:RegisterEvent(ev)
  end
end

function MikCEH.UnregisterCombatParseEvents()
  for _, ev in ipairs(MikCEH.cleuReplacedEvents) do
    MCEHEventFrame:UnregisterEvent(ev)
  end
end
